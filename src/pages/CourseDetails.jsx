...
const CourseDetails = () => {
    ...
    //  console.log(":TOken is Hai", token )
        ...
        // console.log("Hello Jee ai chala hui ")
    }
                    ...
                    <div className='flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden'>
                        <p className='space-x-3 pb-4 text-3xl font-semibold text-richblack-5'>
                            ...
                            >
                            {
                                ...
                                alreadyEnrolled ? <button onClick={()=>{navigate("/dashboard/getEnrolledCourses")}}  className='yellowButton'>Go to Course</button> : <button onClick={()=>handelBuyCourse()} className='bg-yellow-200'>Buy Now</button>
                            }
                            {
                                ...
                                (
                                    ...
                                    (<button onClick={handelAddToCart}  className='border-2 border-white blackButton bg-yellow-200 text-richblack-5'>Add to Cart</button>)
                                )
                            }
                            </div>
                            }
                ...
                <div className='right-[1rem] top-[60px] mx-auto hidden min-h-[600px] w-1/3 max-w-[410px] translate-y-24 md:translate-y-0 lg:absolute  lg:block'>
                    <div className='flex flex-col gap-4 rounded-md bg-richblack-700 p-4 text-richblack-5'>
                        ...
                        <div className='px-4'>
                            ...
                            ₹<span className=''>{courseDetail?.price}</span>
                            </div>
                            <div className='flex flex-col gap-4 justify-center items-center'>
                                ...
                                {
                                    // onClick={handelPayment} 
                                    alreadyEnrolled ? <button onClick={()=>{navigate("/dashboard/getEnrolledCourses")}} className=' bg-yellow-200 flex justify-center items-center w-[100%] rounded-2xl  lg:h-[40px]  text-xl font-bold text-richblack-900'>Go to Course</button> : <button onClick={()=>handelBuyCourse()}  className=' bg-yellow-200 flex rounded-2xl  justify-center items-center w-[100%] lg:h-[40px] text-xl font-bold text-richblack-900'>Buy Now</button>
                                }
                                {
                                alreadyEnrolled ? (<div></div>) : 
                                (
                                    cart?.find((item) => item._id === courseDetail._id) ?
                                    (<button onClick={()=>{navigate("/dashboard/cart")}} className='blackButton  bg-yellow-200 flex justify-center items-center w-[100%] lg:h-[40px] rounded-2xl text-xl font-bold text-richblack-900 '>Go to Cart</button>) :
                                    // 
                                    (<button onClick={handelAddToCart} className='blackButton bg-yellow-200 flex justify-center items-center w-[100%] rounded-2xl lg:h-[40px] text-xl font-bold text-richblack-900'>Add to Cart</button>)
                                )
                            }
                                </>
                                }
